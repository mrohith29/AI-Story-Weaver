    ...
    <form onSubmit={handleSubmit} className="w-full mt-4 animate-fade-in">
      <div className="relative">
        <textarea
          ...
          rows={3}
          disabled={isLoading}
          className="w-full p-4 pr-32 bg-gray-700 border-2 border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-brand-accent focus:outline-none transition-all duration-300 resize-none disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={isLoading || !prompt.trim()}
          className="absolute right-3 top-1/2 -translate-y-1/2 bg-brand-accent hover:bg-sky-500 text-white font-bold py-2 px-4 rounded-lg transition-all duration-300 disabled:bg-gray-500 disabled:cursor-not-allowed flex items-center"
        >
